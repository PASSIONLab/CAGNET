...
X_edge_features = data['distances'][:,:75].reshape(-1, 1)  # Reshape if needed, assuming distances are already in the correct shape
...
class GNNEdgeClassifier(torch.nn.Module):
    def __init__(self, node_input_dim, edge_input_dim, hidden_dim):
        ...
        self.conv1 = GraphConv(node_input_dim, hidden_dim)
        self.bn1 = BatchNorm(hidden_dim)
        self.bn2 = BatchNorm(hidden_dim)
        self.bil = torch.nn.Bilinear(hidden_dim,hidden_dim,1)

        self.conv2 = GraphConv(hidden_dim, hidden_dim)#aggr=aggr.SoftmaxAggregation())
    ...
    def forward(self, data):
        ...
        x = F.gelu(self.conv1(x, edge_index, edge_weight=edge_attr))
        x = self.bn1(x)
        x = F.gelu(self.conv2(x, edge_index, edge_weight=edge_attr))
        x = self.bn2(x)
...
model = GNNEdgeClassifier(node_input_dim, edge_input_dim, hidden_dim)
...
data.edge_index = torch.tensor(data.edge_index, dtype=torch.long) if not isinstance(data.edge_index, torch.Tensor) else data.edge_index
data.edge_attr = torch.tensor(data.edge_attr, dtype=torch.float32) if not isinstance(data.edge_attr, torch.Tensor) else data.edge_attr
...
# Training loop
model.train()
for epoch in range(300):  # Example epoch count
    optimizer.zero_grad()
