...
// 
//             torch::Tensor d_check_tens = torch::zeros({table_size}, torch::TensorOptions()
//                                                                             .dtype(torch::kInt32)
//                                                                             .device(torch::kCUDA));
//             torch::Tensor d_value_tens = torch::zeros({table_size}, torch::TensorOptions()
//                                                                             .dtype(torch::kFloat32)
//                                                                             .device(torch::kCUDA));
//             int *d_check = d_check_tens.data<int>();
//             float *d_value = d_value_tens.data<float>(); // assumes real type is float
// 
// 	    BS = 1024;
// 	    GS = div_round_up(table_size, BS);
// 	    init_check<<<GS, BS, 0, bin->stream[i]>>>(d_check, table_size);
